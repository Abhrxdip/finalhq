...
export function QuestSubmitPage() {
  ...
  const { id } = useParams();
  const questParamId = Array.isArray(id) ? id[0] : id;
  ...
  const quest = questList.find(q => q.id === questParamId) || questList[0] || {
    id: 'quest',
    title: 'Quest',
    category: 'Building',
  ...
  const handleFinalizeSubmit = async () => {
    ...
    const questId = questParamId || quest.id;
  ...
  return (
    <div style={{ fontFamily: fonts.outfit, maxWidth: '740px', margin: '0 auto' }}>
      {/* Back */}
      <button onClick={() => navigate(`/quests/${questParamId || quest.id}`)} style={{ background: 'none', border: 'none', color: colors.textMuted, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', fontSize: '14px', padding: 0, marginBottom: '24px' }}>
      ...
      <div style={{ backgroundColor: colors.bgCard, border: `1px solid ${colors.borderDefault}`, borderRadius: '16px', padding: '32px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '28px', paddingTop: '20px', borderTop: `1px solid ${colors.borderSubtle}` }}>
          <button
            onClick={() => currentStep > 0 ? setCurrentStep(s => s - 1) : navigate(`/quests/${questParamId || quest.id}`)}
